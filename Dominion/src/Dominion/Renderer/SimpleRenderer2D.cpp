...
namespace Dominion {
	...
	static SimpleRenderer2DData s_Data;
	...
	void SimpleRenderer2D::Init()
	{
		...
		s_Data.TextureShader = Shader::Create("TestShader", "assets/Shaders/TestVS.glsl", "assets/Shaders/TestPS.glsl");
	...
	void SimpleRenderer2D::BeginScene(const Camera& camera, const glm::mat4& view)
	{
		DM_PROFILE_FUNCTION();

		s_Data.TextureShader->Bind();
		s_Data.TextureShader->SetMat4("u_ViewProjection", camera.GetProjection() * view);
	}

	void SimpleRenderer2D::BeginScene(const OrthographicCamera& camera)
	{
		DM_PROFILE_FUNCTION();
	...
	void SimpleRenderer2D::DrawQuad(const glm::vec2& position, const glm::vec2& size, const glm::vec4& color)
	{
		DrawQuad({ position.x, position.y, 0.0f }, size, color);
	}

	void SimpleRenderer2D::DrawQuad(const glm::vec3& position, const glm::vec2& size, const glm::vec4& color)
	{
		DM_PROFILE_FUNCTION();
	...
	void SimpleRenderer2D::DrawQuad(const glm::mat4& transform, const glm::vec4& color)
	{
		DM_PROFILE_FUNCTION();
